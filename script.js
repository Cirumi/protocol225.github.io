...
const USERS_BIN_ID = '67e3c5a8ad19ca34f8f1a2c3'; // Бин для хранения всех пользователей
...
let gameMode = 'bot';
let currentUser = null; // { username, password, avatar, code, friends, wins, losses, draws }
...
async function loadAllUsers() {
    try {
        const response = await fetch(`${JSONBIN_BASE}/${USERS_BIN_ID}/latest`, {
...
async function updateUserStats(username, statsUpdate) {
    ...
    if(user) {
        ...
        if(statsUpdate.avatar !== undefined) user.avatar = statsUpdate.avatar;
        await saveAllUsers(data);
    }
}
...
async function handleRegister() {
    ...
    if(username.length < 3) {
        document.getElementById('auth-error').textContent = 'Логин минимум 3 символа';
        return;
    }
...
function onLoginSuccess() {
    localStorage.setItem('protocol225-user', JSON.stringify({ username: currentUser.username, password: currentUser.password }));
...
function logout() {
    localStorage.removeItem('protocol225-user');
    currentUser = null;
    document.getElementById('main-menu').style.display = 'flex';
...
async function updateFriendsList() {
    ...
    if(me) {
        ...
        if(me.friends.length === 0) {
            container.innerHTML = '<p class="empty-text">Нет друзей</p>';
            return;
        }
...
async function addFriendByUI() {
    ...
    try {
        currentUser.friends = await addFriendByCode(currentUser.username, code);
...
async function createOnlineRoom() {
    ...
    try {
        ...
        onlinePollingInterval = setInterval(async () => {
            try {
                const state = await loadFromJSONBin(binId);
                if(state.status==='playing' && onlinePlayerNumber===1) {
        ...
        document.getElementById('online-status').textContent = 'Ошибка: '+e.message;
    }
}
...
function startOnlinePollingForTurn() {
    ...
    onlinePollingInterval = setInterval(async () => {
        try {
            ...
            if(state.turn===onlinePlayerNumber && state.status==='playing') {
                clearInterval(onlinePollingInterval);
                updateFromOnlineState(state);
            }
        } catch(e) {}
    }, 2000);
}
...
async function sendOnlineMove(action) {
    ...
    try {
        ...
        document.getElementById('status-message').textContent = 'Ошибка: '+e.message;
    }
}

// ==================== БОЙ ====================
function resolveBattle(c1, c2) {
    const s1=c1.stars, s2=c2.stars;
...
function playerPlayCard(index) {
    ...
    if(card.class===CLASSES.SUPPORT) { document.getElementById('status-message').textContent='Вспомогательные не реализованы'; return; }
    AudioEngine.play('card');
    const played = playerHand.splice(index,1)[0];
    playedPlayerCard = played;
    addHistory('player', played, '', null);
    playerTurn = false; pendingBattle = true;
    renderAll();
    if(gameMode==='online') { sendOnlineMove({type:'play',card:played.id}); }
    else { setTimeout(()=>enemyPlay(played), 700); }
}

function playerPass() {
    if(!roundActive||!playerTurn||playerPassed||gameOver||pendingBattle) return;
    playerPassed = true;
...
function updateComboSelection() {
    ...
    if(selectedComboCards.length>=2) {
        ...
        if(combo){ info.innerHTML=`<div style="color:#4aff4a;">✅ ${combo.name} (${combo.stars}⭐)</div>`; btn.disabled=false; btn.textContent='СОЗДАТЬ: '+combo.name; }
...
function confirmCombo() {
    ...
    playerHand.push({id:combo.id,name:combo.name,stars:combo.stars,class:combo.class,icon:combo.icon,isCombo:true});
    addHistory('player', playerHand[playerHand.length-1], `Комбо: ${combo.name} (${combo.stars}⭐)`, '');
    showFloatingText('КОМБО!','#ffd700',window.innerWidth/2-40,window.innerHeight/2-60);
    closeComboMenu(); renderAll();
}

// ==================== БОТ ====================
function enemyPlay(playerCard) {
    ...
    let rt='', rtype='';
    if(playerCard){
        const r=resolveBattle(playerCard, playedEnemyCard);
    ...
    lastBattleResult={text:rt,type:rtype};
    finishBattle(rtype);renderAll();
}

function finishBattle(rt){setTimeout(()=>{if(rt==='win')playedEnemyCard=null;if(rt==='lose')playedPlayerCard=null;if(rt==='draw'){playedPlayerCard=null;playedEnemyCard=null;}lastBattleResult=null;pendingBattle=false;renderAll();},rt?1500:500);if(playerPassed&&enemyPassed){setTimeout(()=>endRound(),1500);return;}setTimeout(()=>{if(!gameOver&&roundActive&&!(playerPassed&&enemyPassed)){playerTurn=true;renderAll();}},rt?1600:600);checkWinCondition();}
function checkStarVictory(){const pc=playerHand.some(c=>isCombatCard(c)),ec=enemyHand.some(c=>isCombatCard(c));if(!pc&&!ec){const ps=getTotalStars(playerHand),es=getTotalStars(enemyHand);if(ps>es){gameOver=true;AudioEngine.play('victory');document.getElementById('status-message').textContent=`🏆 ПОБЕДА ПО ЗВЁЗДАМ! (${ps}>${es})`;}else if(es>ps){gameOver=true;AudioEngine.play('lose');document.getElementById('status-message').textContent=`💀 ПОРАЖЕНИЕ (${ps}<${es})`;shakeScreen();}roundActive=false;renderAll();}}
function endRound(){roundActive=false;pendingBattle=false;playedPlayerCard=null;playedEnemyCard=null;lastBattleResult=null;drawForPlayer();drawForEnemy();document.getElementById('battle-result').textContent='';document.getElementById('vs-text').textContent='⚔️';checkWinCondition();renderAll();}
function newRound(){if(gameOver){newGame();return;}roundActive=true;playerPassed=false;enemyPassed=false;playerTurn=true;pendingBattle=false;lastBattleResult=null;playedPlayerCard=null;playedEnemyCard=null;document.getElementById('battle-result').textContent='';document.getElementById('vs-text').textContent='⚔️';renderAll();}
function checkWinCondition(){if(playerHand.length===0&&mainDeck.length===0){gameOver=true;AudioEngine.play('victory');document.getElementById('status-message').textContent='🏆 ПОБЕДА!';if(currentUser)updateUserStats(currentUser.username,{wins:1});}else if(enemyHand.length===0&&mainDeck.length===0){gameOver=true;AudioEngine.play('lose');document.getElementById('status-message').textContent='💀 ПОРАЖЕНИЕ';if(currentUser)updateUserStats(currentUser.username,{losses:1});shakeScreen();}if(!gameOver)checkStarVictory();}
function newGame(){if(onlinePollingInterval)clearInterval(onlinePollingInterval);const all=shuffle(ALL_CARDS);mainDeck=[...all];discardPile=[];playerHand=[];enemyHand=[];for(let i=0;i<6;i++){if(mainDeck.length)playerHand.push(mainDeck.pop());if(mainDeck.length)enemyHand.push(mainDeck.pop());}roundActive=true;playerPassed=false;enemyPassed=false;playerTurn=true;gameOver=false;pendingBattle=false;lastBattleResult=null;turnHistory=[];stats={playerWins:0,enemyWins:0,draws:0};playedPlayerCard=null;playedEnemyCard=null;document.getElementById('battle-result').textContent='';document.getElementById('vs-text').textContent='⚔️';renderAll();}
...
function renderAll() {
    ...
    if(playedPlayerCard) { ps.appendChild(renderBattleCard(playedPlayerCard)); ps.classList.add('active'); } else ps.classList.remove('active');
    ...
    if(lastBattleResult) { br.textContent=lastBattleResult.text; br.className='battle-result '+lastBattleResult.type; vt.textContent=lastBattleResult.type==='win'?'✅':lastBattleResult.type==='lose'?'❌':'💀'; }
    else { br.textContent=''; br.className='battle-result'; vt.textContent='⚔️'; }
    document.getElementById('pass-btn').disabled = !canAct||playerPassed;
    document.getElementById('give-turn-btn').disabled = !canAct;
    document.getElementById('draw-btn').disabled = !roundActive||mainDeck.length===0||playerHand.length>=6||pendingBattle;
    document.getElementById('combo-btn').disabled = !canAct||playerHand.length<2;
    updateStatus();
}
...
function addHistory(actor, card, resultText, resultType) {
    turnHistory.push({actor,card,resultText,resultType});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedUser = localStorage.getItem('protocol225-user');
    if(savedUser) {
        const parsed = JSON.parse(savedUser);
        loginUser(parsed.username, parsed.password).then(user => {
            currentUser = user;
            onLoginSuccess();
        }).catch(() => {
            localStorage.removeItem('protocol225-user');
