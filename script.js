...
async function loadAllUsers() {
    ...
    } catch(e) { console.error('Ошибка загрузки пользователей:', e); return { users: [] }; }
}

async function saveAllUsers(usersData) {
    try {
        const response = await fetch(`${JSONBIN_BASE}/${USERS_BIN_ID}`, {
            method: 'PUT', headers: { 'Content-Type':'application/json', 'X-Master-Key':JSONBIN_API_KEY, 'X-Bin-Private':'false' },
            body: JSON.stringify(usersData)
        });
        if(!response.ok) throw new Error('Ошибка сохранения');
    } catch(e) { console.error('Ошибка сохранения пользователей:', e); throw e; }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedLogin = localStorage.getItem('protocol225-user-login');
    if(savedLogin) document.getElementById('auth-username').value = savedLogin;
