...
const COMBO_RECIPES = [
    { id:'combo_infantry', name:'Отряд пехоты', icon:'🪖', stars:3, class:CLASSES.MILITARY, cards:['m8','m7','m5'] },
    { id:'combo_elite', name:'Элитный спецназ', icon:'🎖️', stars:6, class:CLASSES.MILITARY, cards:['m4','m6','m3'] },
    { id:'combo_armor', name:'Бронеколонна', icon:'🚙', stars:8, class:CLASSES.MILITARY, cards:['m1','m2','m4'] },
    { id:'combo_terror', name:'Террор-ячейка', icon:'💣', stars:7, class:CLASSES.CRIMINAL, cards:['c9','c8','c5'] },
    { id:'combo_psycho', name:'Псих-отряд', icon:'🔥', stars:8, class:CLASSES.CRIMINAL, cards:['c6','c7','c4'] },
    { id:'combo_medical', name:'Мед-бригада', icon:'🚑', stars:9, class:CLASSES.SUPPORT, cards:['s1','s3','s5'], isSupportCombo:true, bonusStars:3 },
    { id:'combo_engineering', name:'Инженерный корпус', icon:'🔧', stars:7, class:CLASSES.SUPPORT, cards:['s4','s2','m3'], isSupportCombo:true, bonusStars:2 }
];

// ==================== API КОНФИГ ====================
const JSONBIN_API_KEY = '$2a$10$CKMC.ahtZuFY9e5STCeTOerv6m/MBp9zZIZajplMIqrvgSzozEBuW';
const JSONBIN_BASE = 'https://api.jsonbin.io/v3/b';
const USERS_BIN_ID = '6a12e5eeee5a733b12100ffd';
...
let currentTheme = 'cyberpunk', gameMode = 'bot';
let currentUser = null;
let onlineRoomId = null, onlinePlayerNumber = 0, onlinePollingInterval = null;
let playerUsedPass = false, playerSupportBonus = 0;
...
// ==================== АККАУНТЫ ====================
async function apiCall(method, binId, body) {
...
async function loadAllUsers() {
    try {
        const data = await apiCall('GET', USERS_BIN_ID + '/latest');
...
async function saveAllUsers(usersData) {
    await apiCall('PUT', USERS_BIN_ID, usersData);
}
...
async function registerUser(username, password) {
    ...
    const newUser = {
        username, password, avatar:'👤', code:generateCode(),
        friends:[], wins:0, losses:0, draws:0,
...
async function updateUserStats(username, statsUpdate) {
    ...
    if(user) {
        if(statsUpdate.wins) user.wins += statsUpdate.wins;
        if(statsUpdate.losses) user.losses += statsUpdate.losses;
        if(statsUpdate.draws) user.draws += statsUpdate.draws;
        if(statsUpdate.friends) user.friends = statsUpdate.friends;
        await saveAllUsers(data);
    }
}
...
function showLoginScreen() {
    ...
    document.getElementById('auth-username').value = localStorage.getItem('protocol225-user-login')||'';
}
...
async function handleLogin() {
    ...
    if(!u||!p) { document.getElementById('auth-error').textContent='Заполните все поля'; return; }
    ...
    try {
        ...
        document.getElementById('user-stats-display').textContent = `Побед: ${currentUser.wins} | Поражений: ${currentUser.losses}`;
...
async function handleRegister() {
    ...
    const p = document.getElementById('auth-password').value;
    if(!u||!p) { document.getElementById('auth-error').textContent='Заполните все поля'; return; }
    if(u.length<3) { document.getElementById('auth-error').textContent='Логин минимум 3 символа'; return; }
    if(p.length<4) { document.getElementById('auth-error').textContent='Пароль минимум 4 символа'; return; }
    document.getElementById('auth-error').textContent='Регистрация...';
    try {
        currentUser = await registerUser(u, p);
        localStorage.setItem('protocol225-user-login', currentUser.username);
        document.getElementById('auth-menu').style.display='none';
        document.getElementById('main-menu').style.display='flex';
        document.getElementById('user-info-display').textContent = currentUser.avatar+' '+currentUser.username;
        document.getElementById('user-stats-display').textContent = `Побед: 0 | Поражений: 0`;
        document.getElementById('my-code-display').textContent = currentUser.code;
        document.getElementById('login-btn').style.display='none';
        document.getElementById('register-btn-menu').style.display='none';
        document.getElementById('logout-btn').style.display='inline-block';
...
function logout() {
    ...
    document.getElementById('friends-list-container').innerHTML='<p class="empty-text">Войдите для списка друзей</p>';
}
...
async function updateFriendsList() {
    ...
    if(me) {
        ...
        c.innerHTML = me.friends.map(f=>`
            <div class="friend-item"><span>${f.username} (${f.code})</span>
            <div><button onclick="challengeFriend('${f.code}')">⚔️</button><button onclick="removeFriendFromList('${f.code}')">❌</button></div></div>
        `).join('');
    }
}
...
async function createOnlineRoom() {
    ...
    try {
        ...
        const ids = shuffle(ALL_CARDS).map(c=>c.id);
        const state = {
            p1:{ n:currentUser?.username||'Игрок1', h:ids.splice(0,6), pass:false },
            p2:{ n:'Ожидание...', h:[], pass:false },
            d:ids, dp:[], t:1, s:'waiting'
        };
        const res = await apiCall('POST', null, state);
        onlineRoomId = res.metadata?.id || res.id;
        document.getElementById('room-code-display').textContent=onlineRoomId;
        document.getElementById('room-created-info').style.display='block';
        document.getElementById('online-status').textContent='Ожидание игрока...';
        pollOnlineRoom(onlineRoomId);
...
async function joinOnlineRoom() {
    ...
    try {
        const state = await apiCall('GET', rid+'/latest');
        const data = state.record;
        if(data.s!=='waiting') { document.getElementById('online-status').textContent='Комната занята'; return; }
        onlineRoomId=rid; onlinePlayerNumber=2;
        const ids = [...data.d];
        data.p2 = { n:currentUser?.username||'Игрок2', h:ids.splice(0,6), pass:false };
        data.d=ids; data.s='playing'; data.t=1;
        await apiCall('PUT', rid, data);
        document.getElementById('online-status').textContent='Подключено!';
        setTimeout(()=>startOnlineGame(data),500);
    } catch(e) { document.getElementById('online-status').textContent='Ошибка: '+e.message; }
}

function pollOnlineRoom(rid) {
    if(onlinePollingInterval) clearInterval(onlinePollingInterval);
    onlinePollingInterval = setInterval(async ()=>{
        try {
            const res = await apiCall('GET', rid+'/latest');
            const data = res.record;
            if(data.s==='playing' && onlinePlayerNumber===1) {
...
function startOnlineGame(data) {
    gameMode='online';
    const map={}; ALL_CARDS.forEach(c=>map[c.id]={...c});
    mainDeck=data.d.map(id=>map[id]).filter(c=>c);
    discardPile=(data.dp||[]).map(id=>map[id]).filter(c=>c);
    const me = onlinePlayerNumber===1?data.p1:data.p2;
    playerHand=me.h.map(id=>map[id]).filter(c=>c);
    enemyHand=[];
    document.getElementById('main-menu').style.display='none';
    ...
    playerTurn=(data.t===onlinePlayerNumber); pendingBattle=false;
    ...
    renderAll();
    if(!playerTurn) pollOnlineTurn();
}

function pollOnlineTurn() {
    if(onlinePollingInterval) clearInterval(onlinePollingInterval);
    onlinePollingInterval = setInterval(async ()=>{
        try {
            const res = await apiCall('GET', onlineRoomId+'/latest');
            const data = res.record;
            if(data.t===onlinePlayerNumber && data.s==='playing') {
                clearInterval(onlinePollingInterval);
                updateFromOnline(data);
            }
        } catch(e) {}
    }, 2000);
}

function updateFromOnline(data) {
    const map={}; ALL_CARDS.forEach(c=>map[c.id]={...c});
    mainDeck=data.d.map(id=>map[id]).filter(c=>c);
    discardPile=(data.dp||[]).map(id=>map[id]).filter(c=>c);
    const me = onlinePlayerNumber===1?data.p1:data.p2;
    const en = onlinePlayerNumber===1?data.p2:data.p1;
    playerHand=me.h.map(id=>map[id]).filter(c=>c);
    if(en.h) enemyHand=en.h.map(id=>map[id]).filter(c=>c);
    playerPassed=me.pass; enemyPassed=en.pass;
...
async function sendOnlineMove(action) {
    ...
    try {
        const res = await apiCall('GET', onlineRoomId+'/latest');
        const data = res.record;
        if(onlinePlayerNumber===1) { data.p1.h=playerHand.map(c=>c.id); data.p1.pass=playerPassed; }
        else { data.p2.h=playerHand.map(c=>c.id); data.p2.pass=playerPassed; }
        data.d=mainDeck.map(c=>c.id); data.dp=discardPile.map(c=>c.id);
        data.t=onlinePlayerNumber===1?2:1;
        await apiCall('PUT', onlineRoomId, data);
        playerTurn=false; renderAll();
        pollOnlineTurn();
    } catch(e) { document.getElementById('status-message').textContent='Ошибка сети'; }
}

// ==================== БОЙ ====================
function resolveBattle(c1, c2, b1=0, b2=0) {
    const s1=c1.stars+b1, s2=c2.stars+b2;
    if(isAgentCard(c1)&&isAgentCard(c2)) return {w:'draw',t:'Агенты уничтожают друг друга'};
    if(isAgentCard(c1)) return {w:'c1',t:'Агент ликвидирует цель',st:true,aa:c1.isAntiAgent||false};
    if(isAgentCard(c2)) return {w:'c2',t:'Агент ликвидирует цель',st:true,aa:c2.isAntiAgent||false};
    if(c1.isLiza) return {w:'c1',t:'Лиза² уничтожает (225⭐)'};
    if(c2.isLiza) return {w:'c2',t:'Лиза² уничтожает (225⭐)'};
    if(c1.class===CLASSES.CRIMINAL&&c1.stars>=3&&c1.stars<=5&&c2.class===CLASSES.MILITARY&&c2.stars>=1&&c2.stars<=3) return {w:'c1',t:`Сильный преступник (${s1}⭐) одолел (${s2}⭐)`};
    if(c2.class===CLASSES.CRIMINAL&&c2.stars>=3&&c2.stars<=5&&c1.class===CLASSES.MILITARY&&c1.stars>=1&&c1.stars<=3) return {w:'c2',t:`Сильный преступник (${s2}⭐) одолел (${s1}⭐)`};
    if(c1.class===CLASSES.MILITARY&&c2.class===CLASSES.CRIMINAL) return {w:'c1',t:'Военные > Преступников'};
    if(c2.class===CLASSES.MILITARY&&c1.class===CLASSES.CRIMINAL) return {w:'c2',t:'Военные > Преступников'};
    if(c1.class===CLASSES.CRIMINAL&&c2.class===CLASSES.CIVILIAN) return {w:'c1',t:'Преступники > Мирных'};
    if(c2.class===CLASSES.CRIMINAL&&c1.class===CLASSES.CIVILIAN) return {w:'c2',t:'Преступники > Мирных'};
    if(s1>s2) return {w:'c1',t:`Больше звёзд (${s1}>${s2})`};
    if(s2>s1) return {w:'c2',t:`Больше звёзд (${s2}>${s1})`};
    if(c1.class===c2.class) return {w:'draw',t:'Один. класс + звёзды — ничья'};
    return {w:'draw',t:'Равные звёзды — ничья'};
}

function stealCard(fromHand, isAntiAgent, cb) {
    if(fromHand.length===0) { if(cb)cb(null); return; }
    if(isAntiAgent) { showStealMenu(fromHand, i=>{ if(i===null){cb(null);return;} cb(fromHand.splice(i,1)[0]); }); }
    else { cb(fromHand.splice(Math.floor(Math.random()*fromHand.length),1)[0]); }
}
...
function playerPlayCard(index) {
    ...
    if(card.class===CLASSES.SUPPORT) {
        ...
        playerSupportBonus++; discardPile.push(playerHand.splice(index,1)[0]);
        AudioEngine.play('card'); renderAll(); return;
    }
    if(card.isSupportCombo) {
        if(!playedPlayerCard) { document.getElementById('status-message').textContent='Сначала выложите боевую карту'; return; }
        playerSupportBonus+=card.bonusStars; discardPile.push(playerHand.splice(index,1)[0]);
        AudioEngine.play('card'); renderAll(); return;
    }
    AudioEngine.play('card');
    const played = playerHand.splice(index,1)[0];
    playedPlayerCard=played; playerTurn=false; pendingBattle=true;
    renderAll();
...
function giveTurn() {
    if(!roundActive||!playerTurn||playerUsedPass||gameOver||pendingBattle) return;
    playerUsedPass=true; playerTurn=false;
    renderAll();
    if(gameMode==='online') sendOnlineMove({type:'pass'});
...
function playerDraw() {
    ...
    AudioEngine.play('drawCard'); drawForPlayer(); renderAll();
}
...
function updateComboSelection() {
    ...
    if(selectedComboCards.length>=2) {
        ...
        if(combo){ info.innerHTML=`<div style="color:#4aff4a;">✅ ${combo.name} (${combo.stars}⭐)</div>`; btn.disabled=false; btn.textContent='СОЗДАТЬ'; }
        else { info.innerHTML='<div style="color:#ffaa00;">⚠️ Нет комбинации</div>'; btn.disabled=true; btn.textContent='НЕТ КОМБО'; }
    } else { info.innerHTML='<div style="color:#888;">Выберите 2-3 карты</div>'; btn.disabled=true; btn.textContent='ВЫБЕРИТЕ'; }
    renderComboTable();
}
function renderComboTable() {
    const container=document.getElementById('combo-table-container');
    if(!container) return;
    let html='<table class="combo-table"><thead><tr><th>Комбо</th><th>Карты</th><th>⭐</th></tr></thead><tbody>';
    COMBO_RECIPES.forEach(combo=>{
        const names=combo.cards.map(id=>{ const c=ALL_CARDS.find(x=>x.id===id); return c?`${c.icon}${c.name}`:'???'; }).join(' + ');
        const hasAll=combo.cards.every(id=>playerHand.some(c=>c.id===id));
        html+=`<tr class="${hasAll?'combo-available':''}"><td>${combo.icon} <b>${combo.name}</b></td><td>${names}</td><td style="color:#ffd700;">${combo.stars}⭐${combo.isSupportCombo?'(+'+combo.bonusStars+')':''}</td></tr>`;
    });
...
function confirmCombo() {
    ...
    playerHand.push({id:combo.id,name:combo.name,stars:combo.stars,class:combo.class,icon:combo.icon,isCombo:true,isSupportCombo:combo.isSupportCombo||false,bonusStars:combo.bonusStars||0});
...
function enemyPlay(playerCard, playerBonus=0) {
    ...
    if(supportCards.length>0&&!isAgentCard(playedEnemyCard)&&!playedEnemyCard.isLiza){ enemyHand.splice(enemyHand.indexOf(supportCards[0]),1); enemyBonus=1; }
    AudioEngine.play('card');
    let rt='', rtype='';
    if(playerCard){
        const r=resolveBattle(playerCard, playedEnemyCard, playerBonus, enemyBonus);
        if(r.w==='c1'){rt='Победа: '+r.t;rtype='win';stats.playerWins++;AudioEngine.play('win');discardPile.push(playedEnemyCard);playedEnemyCard=null;if(r.st&&enemyHand.length>0){stealCard(enemyHand,r.aa,(s)=>{if(s)playerHand.push(s);finishBattle(rtype);});renderAll();return;}}
        else if(r.w==='c2'){rt='Поражение: '+r.t;rtype='lose';stats.enemyWins++;AudioEngine.play('lose');shakeScreen();discardPile.push(playerCard);playedPlayerCard=null;if(r.st&&playerHand.length>0){stealCard(playerHand,true,(s)=>{if(s)enemyHand.push(s);finishBattle(rtype);});renderAll();return;}}
        else {rt='Ничья: '+r.t;rtype='draw';stats.draws++;AudioEngine.play('draw');discardPile.push(playerCard);discardPile.push(playedEnemyCard);playedPlayerCard=null;playedEnemyCard=null;}
...
function endRound(){
    ...
    playerSupportBonus=0;playerUsedPass=false; drawForPlayer();drawForEnemy();
    document.getElementById('battle-result').textContent='';document.getElementById('vs-text').textContent='⚔️'; checkWinCondition();renderAll();
}

function newRound(){
    if(gameOver){newGame();return;}
    roundActive=true;playerPassed=false;enemyPassed=false;playerTurn=true;pendingBattle=false;
    lastBattleResult=null;playedPlayerCard=null;playedEnemyCard=null;playerSupportBonus=0;playerUsedPass=false;
...
function renderBattleCard(card, bonus=0) {
    ...
    div.innerHTML=`${card.isCombo?'<div style="position:absolute;top:2px;right:2px;font-size:8px;">🔗</div>':''}<div class="card-icon">${card.icon}</div><div class="card-name">${card.name}</div><div class="card-stars">${'⭐'.repeat(Math.min(card.stars,5))}${card.stars>5?' '+card.stars:''}</div>${bonus>0?`<div style="color:#4aff4a;font-size:9px;position:absolute;bottom:2px;">+${bonus}⭐</div>`:''}<div class="card-class">${getCardClassText(card)}</div>`;
    return div;
}
...
function renderAll() {
    ...
    enemyHand.forEach(c=>document.getElementById('enemy-cards').appendChild(renderCard(c,true)));
    ...
    if(lastBattleResult){br.textContent=lastBattleResult.text;br.className='battle-result '+lastBattleResult.type;vt.textContent=lastBattleResult.type==='win'?'✅':lastBattleResult.type==='lose'?'❌':'💀';}else{br.textContent='';br.className='battle-result';vt.textContent='⚔️';}
...
function updateStatus() {
    ...
    else if(!roundActive)s.textContent='Раунд завершён';
    ...
    else s.textContent=gameMode==='online'?'Ожидание...':'Ход компьютера...';
}
...
document.addEventListener('DOMContentLoaded',()=>{
    setTheme(localStorage.getItem('protocol225-theme')||'cyberpunk');
    if(localStorage.getItem('protocol225-user-login')) document.getElementById('auth-username').value=localStorage.getItem('protocol225-user-login');
    ...
    document.querySelectorAll('.theme-option').forEach(b=>b.addEventListener('click',()=>{setTheme(b.dataset.theme);document.getElementById('theme-selector').classList.remove('active');}));
